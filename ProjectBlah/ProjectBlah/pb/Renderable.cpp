...
namespace pb
{
	Renderable::Renderable(const char* name) : GameObject(name),
		attribute_config({
			...
			{ Material::REQUIREMENTS_ATTRIBUTE_INDEX, attribute_data{ 1, GL_ELEMENT_ARRAY_BUFFER, GL_UNSIGNED_INT, 1, 1, sizeof(unsigned int), -1, &((*new vector<unsigned int>{ 0 })[0]) } },
			{ Material::REQUIREMENTS_ATTRIBUTE_UV, attribute_data{ 2, GL_ARRAY_BUFFER, GL_FLOAT, 1, 2, sizeof(vec2), -1, &((*new vector<vec2>{ vec2(0, 1) })[0]) } }
	}),
		uniform_config({
	...
	void Renderable::Draw()
	{
		...
		{
			*static_cast<mat4*>(uniform_config[Material::REQUIREMENTS_UNIFORM_MVP].data_pointer) =
				Camera::GetActiveCamera()->GetProjection() *  Camera::GetActiveCamera()->GetView() * transform_matrix_world;
		}
